...
Integration follow-up: merged the reviewed foundation containing current main `6673f2f` and its linked-scene/share reload fixes. Editable collection links preserve existing work through the common load flow; embedded previews are inert. The paper scene and optical model are unchanged. Regenerated collection pages and verified the full suite, JavaScript syntax and patch checks after resolving the integration.
