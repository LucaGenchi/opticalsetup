// Render research pages and link any individually reviewed native scenes.
...
const setupIds = new Set((await readdir(join(DIR, 'setups')).catch(() => []))
...
const setupEnd = `<footer>Paper evidence and an individually reviewed native reconstruction. Source PDFs remain with their authors and publishers.<br>
<a href="/collections/2pp/sources.json">Download source manifest</a> · <a href="/collections/2pp/papers.json">Download research records</a></footer></body></html>\n`;
...
<div class="summary"><strong>17 references · research preserved · individual setups rebuilt</strong><p>Fourteen references have usable primary documents. Entries gain a working native scene only after a paper-specific reconstruction. Gu is based on the official supplement and public apparatus figures. The GT datasheet is a 2016 revision of a benchmark labelled 2014. Dong, Yang and Yan still need full text.</p></div>
...
for (const p of records.papers) {
  ...
  const hasResearchNote = await exists(join(DIR, 'research', `${p.id}.md`));
  ...
  const setupBlock = hasSetup ? `<div class="summary"><strong>Working native setup</strong><p>The editable scene below traces the reported writing and observation paths with explicitly labelled interpretation where prescriptions are unavailable.</p></div>
<div class="actions"><a class="button" href="/sketch/?paper=${esc(p.id)}">Open editable setup</a><a class="button secondary" href="/collections/2pp/setups/${esc(p.id)}.json" download>Download native JSON</a>${hasResearchNote ? `<a class="download" href="/collections/2pp/research/${esc(p.id)}.md">Evidence and controls</a>` : ''}</div>
<section><h2>Live native preview</h2><div class="preview"><iframe src="/sketch/?paper=${esc(p.id)}&amp;embed=1" title="Interactive OpticalSetup preview for ${esc(pretty(p.id))}" loading="lazy"></iframe></div><p class="caption">Computed rays, native optics and live scanner motion. Click a component to inspect it; open the editable copy to change controls.</p></section>`
  ...
  const handoffBlock = handoff?.url ? `<section><h2>2PP Lab handoff</h2><div class="handoff"><div><p>The paper-based handoff preserves only values accepted by the companion calculator.</p><a class="button secondary" href="${esc(handoff.url)}">Continue with supported paper values</a></div><div><h3>Imported</h3><ul>${handoff.imported.map(f => `<li>${esc(f.label)}: ${esc(f.value)} ${esc(f.unit)}</li>`).join('')}</ul><h3>Not imported</h3><ul>${handoff.omitted.map(f => `<li>${esc(f.label)}: ${esc(f.value ?? 'unknown')} ${esc(f.unit)} — ${esc(f.reason)}</li>`).join('')}</ul></div></div><p class="caption">This transfer is not a calibrated multi-focus cure prediction.</p></section>` : '';
...
<section><h2>Limits and unresolved details</h2><p>${esc(p.modelLimits)}</p><ul>${list(p.unknowns)}</ul>${settings ? `<details><summary>Reported numerical inputs and units</summary><table><tbody>${settings}</tbody></table></details>` : ''}</section>${handoffBlock}</main>` + (hasSetup ? setupEnd : end);
  await mkdir(join(DIR, p.id), { recursive:true });
  await writeFile(join(DIR, p.id, 'index.html'), html);
}
console.log(`Built ${records.papers.length} research pages and linked available native setups`);
