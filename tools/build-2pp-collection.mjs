// Render the preserved research notes and any individually reviewed native
// scenes. A scene is opt-in per paper record; siblings remain research-only.
...
const rows = records.papers.map(p => `<tr><td>${p.year}</td><th scope="row"><a href="${p.id}/">${esc(pretty(p.id))}</a><span>${esc(p.title)}</span></th><td>${esc(p.family)}</td><td>${p.scene ? 'Working native scene' : p.status === 'reviewed' ? 'Source notes available' : 'Full text needed'}</td></tr>`).join('\n');
...
<section><h2>One apparatus at a time</h2><p>The previous batch-generated setups were removed. Each reference retains its reported optical sequence, inspected evidence, parameters and open questions. New native scenes appear only after a paper-specific source review and verification.</p><p><a href="${esc(records.sourceArticle)}">Original throughput-scaling article</a></p></section></main>` + end);
...
for (const p of records.papers) {
  ...
  const setupSummary = p.scene
    ? '<div class="summary"><strong>Working native scene · paper-specific review</strong><p>The editable scene below traces the reported writing sequence and labels every interpretation and unsupported physical claim.</p></div>'
  ...
  const setupSection = p.scene ? `<section><h2>Working OpticalSetup scene</h2>
<div class="actions"><a class="button" href="/sketch/?collection=${esc(p.id)}">Open editable workbench</a><a class="button secondary" href="../${esc(p.scene)}" download>Download native scene</a>${p.researchNote ? `<a class="download" href="../${esc(p.researchNote)}">Evidence and controls</a>` : ''}</div>
<div class="preview"><iframe title="${esc(pretty(p.id))} working optical setup" loading="lazy" src="/sketch/?collection=${esc(p.id)}&amp;locked=1"></iframe></div>
<p class="caption">Live native scene: click components to inspect them. Open the editable workbench to change controls, save, reload, or export.</p></section>` : '';
...
console.log(`Built ${records.papers.length} research pages; ${records.papers.filter(p => p.scene).length} working scene(s) linked`);
