...
for (const paper of records.papers) {
  if (await exists(join(DIR, 'setups', `${paper.id}.json`))) setupIds.add(paper.id);
}
...
<div class="summary"><strong>17 references · ${setupIds.size} working native setup${setupIds.size === 1 ? '' : 's'}</strong><p>Fourteen references have usable primary documents. Reconstructed scenes are added one paper at a time after source and browser verification; remaining records retain research notes only.</p></div>
...
for (const p of records.papers) {
  ...
  const setup = `/collections/2pp/setups/${p.id}.json`;
  const note = `/collections/2pp/research/${p.id}.md`;
  const actions = hasSetup ? `<div class="actions"><a class="button" href="/sketch/?paper=${esc(p.id)}">Open editable setup</a><a class="button secondary" href="${esc(setup)}" download>Download native JSON</a><a class="download" href="${esc(note)}">Evidence and controls</a></div>
<div class="preview"><iframe title="${esc(p.title)} working setup" src="/sketch/?paper=${esc(p.id)}&amp;embed=1" loading="lazy"></iframe></div><p class="caption">Live native OpticalSetup scene. Click components to inspect reported values and explicitly labelled interpretation.</p>` : '';
...
<div class="summary"><strong>${hasSetup ? 'Working native reconstruction' : 'Research retained · setup removed'}</strong><p>${hasSetup ? 'This source-checked scene traces the reported functional train with its interpretation and simulation limits labelled in the workbench.' : p.status === 'reviewed' ? 'The generated drawing has been removed. The source review below is preserved for a future individual reconstruction.' : 'Full text is still needed. The abstract and bibliographic record do not establish the complete apparatus.'}</p></div>
...
console.log(`Built ${records.papers.length} research pages with ${setupIds.size} working setup${setupIds.size === 1 ? '' : 's'}`);
