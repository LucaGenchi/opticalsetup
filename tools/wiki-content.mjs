  ...
  },
  {
    ...
    realWorld: {
      ...
      html4: `
        ...
        <p>A bulk crystal or glass does the same with a focused femtosecond beam: above the critical power for self-focusing the beam collapses into a filament, and self-phase modulation in it broadens the spectrum${cite(19)}. Where the spectrum ends depends mostly on the medium and the pump wavelength. The blue edge is set chiefly by the bandgap, since multiphoton absorption clamps the filament's intensity, so wide-bandgap media reach furthest into the blue; with near-infrared pumping it moves little with pulse energy. The red side grows with the pump wavelength. Pumped near 800 nm, sapphire typically spans about 410–1100 nm, YAG about 420–1600 nm, fused silica about 390–1000 nm and CaF₂ about 300–2000 nm${cite(19)}. The near-infrared half is useful in its own right: a 10 mm YAG plate pumped at 1035 nm has provided a 1050–1300 nm Stokes band for multiplex CARS microscopy${cite(20)}.</p>`,
      html5: `
    ...
    inOpticalSetup: {
      html: `<p>Choose a conversion mode before expecting any output: with the default <em>None</em> the crystal does not interact with light at all. Every converting mode is a wavelength-and-power proxy that converts a fixed, authored fraction of the eligible incident light, whatever its intensity, and <em>Transmit residual pump</em> can keep the unconverted remainder on the same path; no residual branch is drawn at efficiencies of 99.9 % or more, and in non-OPO modes it is also omitted when the output centre wavelength equals the input centre wavelength.</p>
        ...
        <p><strong>Supercontinuum</strong> replaces the converted light with a flat band. By default its edges are estimated from the pump that arrives and the chosen medium — YAG, sapphire, fused silica or CaF₂ — by interpolating between published spectra measured at a few pump wavelengths, so a 1035 nm pump in YAG gives roughly 470–1920 nm; the <em>Continuum</em> readout gives the band and says when the pump lies outside the measured wavelengths. A continuous-wave beam lacks the peak power to start a continuum and passes unconverted. <em>Set manually</em> draws an authored band from any pump instead; scenes saved before the estimate existed open with their old 430–870 nm band as a manual range. <strong>Custom output</strong> is an authored output rather than a named physical process: it emits a single line at the entered wavelength, whatever the pump's bandwidth.</p>
    ...
    citations: [
      ...
      { label: 'A. Dubietis, G. Tamošauskas, R. Šuminas, V. Jukna, A. Couairon, “Ultrafast supercontinuum generation in bulk condensed media,” invited review, arXiv:1706.04356 (2017)', url: 'https://arxiv.org/abs/1706.04356' },
