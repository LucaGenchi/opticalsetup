  ...
  },
  {
    ...
    realWorld: {
      ...
      formulas: [
        ...
        { tex: '\\frac{P_s}{P_i} = \\frac{\\nu_s}{\\nu_i} = \\frac{\\lambda_i}{\\lambda_s}', caption: 'Manley–Rowe: signal and idler carry equal photon fluxes, so their powers divide in proportion to photon energy.' },
      ],
      html2: `
        <p>Because the photon fluxes are equal, the longer-wavelength output always receives the smaller share of the power. Pumped at 532 nm with an 800 nm signal, the idler lies at 1588 nm and receives about a third of the converted power. The penalty grows with the wavelength ratio: producing 5 µm light from a 1 µm pump can use at most about 20 % of the pump power${cite(4)}.</p>
      ...
      html3: `
        <p><strong>Linewidth.</strong> The frequencies must still add up, so the width of the pump spectrum has to go somewhere. The cavity holds the resonant signal, and the non-resonant idler takes up whatever the pump and signal leave. Pump frequency fluctuations that the signal does not follow appear on the idler, so imposing them on the signal is a way to remove them from the idler${cite(5)}. For Gaussian spectra the idler's frequency width is therefore the quadrature sum of the pump and signal widths.</p>
        <p><strong>Pulsed OPOs</strong> are what most laboratories use. They fall into three families:</p>
        <ul>
          <li><strong>Synchronously pumped fs and ps OPOs.</strong> The cavity round trip is matched to the repetition period of a mode-locked pump, so each signal pulse is amplified by the next pump pulse. The output stays locked to the pump train, with a pulse duration and spectral width close to the pump's. A Ti:sapphire-pumped fs OPO delivers typically 200 fs signal pulses tunable from 1000 to 1600 nm, with idler from 1750 to 4000 nm, at 80 MHz${cite(6)}. A picosecond system for coherent Raman microscopy delivers 2 ps pulses whose signal has the same 10 cm⁻¹ spectral width as its 1032 nm beam${cite(7)}.</li>
          <li><strong>Nanosecond OPOs</strong> pumped by Q-switched lasers are the high-energy workhorses. Their linewidth is set by the crystal's phase-matching bandwidth and the cavity, not by the pump: a 355 nm-pumped system tunes a signal from 410 to 710 nm and an idler from 710 to 2600 nm in 3–5 ns pulses, with a linewidth below 5 cm⁻¹${cite(8)}. The oscillation needs time to build up from noise during each pump pulse, which raises the threshold${cite(4)} and makes the output pulses somewhat shorter than the pump's${cite(2)}.</li>
          <li><strong>Continuous-wave singly resonant OPOs</strong> resonate a single longitudinal mode of the signal and pass the pump's frequency fluctuations on to the idler${cite(5)}.</li>
        </ul>`,
    },
    inOpticalSetup: {
      html: `<p>Choose a conversion mode before expecting an output: the default <em>None</em> passes light unchanged. SHG and THG set the wavelength to one half or one third of the input. Custom output uses the entered wavelength; supercontinuum creates a fixed illustrative band. Conversion efficiency sets the converted ray weight, and residual-pump transmission retains the unconverted branch. The example doubles 1064 nm to 532 nm and separates it from the remaining pump with a dichroic.</p>
        <p><strong>OPO mode</strong> models a singly resonant oscillator. Set the pump wavelength the crystal is phase-matched for and the resonant signal wavelength; the idler is shown as a readout. Light is converted when its centre lies within 1 nm of the pump wavelength, or within its own spectral width if that is wider, so a femtosecond pump tuned slightly off design still converts. The idler follows the arriving pump by energy conservation while the signal stays where the cavity holds it. Light at any other wavelength, including signal and idler returning through the crystal, passes unconverted. The converted power is divided by Manley–Rowe, and a signal at exactly twice the pump wavelength emits one degenerate beam.</p>
        <p><strong>Pulsed and broadband pumps.</strong> Signal and idler carry their own Gaussian spectra, and every width is handled in wavenumber. With <em>Signal linewidth: follows the pump</em>, for a synchronously pumped fs or ps OPO, the signal is as wide in frequency as the pump. With <em>Set by the cavity</em>, for a ns or CW OPO, it takes the entered width in cm⁻¹; 0 gives a single-frequency signal. The idler width is the quadrature sum of the pump and signal widths, so a dichroic, filter, or spectrometer downstream acts on the new light rather than on the pump's spectrum. Both outputs keep the pump's pulse train: repetition rate, phase, duration, and any modulation gates. They start with no chirp, and only a signal that follows a transform-limited pump is marked transform-limited.</p>
        <p><strong>Threshold.</strong> With the threshold left at 0, OPO mode converts the fixed efficiency fraction, as older saved setups expect. Enter a threshold in watts of average pump power and the crystal compares it with the source's average power, scaled by the fraction of the beam that reaches it. Below threshold nothing oscillates and the pump passes. Above it, the converted fraction is the efficiency multiplied by the plane-wave depletion shown above. The inspector reports the pump level relative to threshold, the depletion, and the output linewidths.</p>`,
      limitations: `<p>Conversion is a wavelength-and-power proxy. The crystal does not solve phase matching from material data, so the signal wavelength and pump acceptance are authored rather than calculated, and neither crystal choice nor temperature affects them. OPO mode does not trace cavity round trips, synchronisation to the pump repetition rate, build-up time, group-velocity walk-off, or spatial overlap of Gaussian beams. Output pulses copy the pump duration, so ns pulse shortening and fs pulse lengthening are not reproduced. Depletion follows the plane-wave curve and is held at its maximum rather than showing back-conversion. Spectra are Gaussian in wavelength with the width implied by the wavenumber calculation, and carry no spectral phase; a filtered pump spectrum contributes only its width.</p>`,
    },
    citations: [
      { label: 'Thorlabs — BBO Crystals for Second Harmonic Generation', url: 'https://www.thorlabs.com/bbo-crystals-for-second-harmonic-generation' },
      ...
      { label: 'APE — picoEmerald brochure and specifications (Rev. 4.1.0, 2020)', url: 'https://www.ape-berlin.de/content/uploads/2020/09/APE-Broch%C3%BCre-picoEmerald-rev.4.1.0.pdf' },
    ...
    related: ['sample', 'dichroic', 'filter', 'pulsedlaser', 'spectrometer'],
  },
  {
