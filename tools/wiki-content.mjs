  ...
  },
  {
    ...
    realWorld: {
      ...
      html4: `
        ...
        <p><strong>Supercontinuum generation</strong> turns intense pulses into a broad continuum, often spanning hundreds of nanometres, rather than a new discrete line${cite(10)}. Early demonstrations came in 1970, when Alfano and Shapiro broadened picosecond pulses in glasses and crystals${cite(11, 12)}. In glass and optical fibers the broadening arises mainly from third-order effects: depending on pump duration and dispersion it can involve self-phase modulation, modulation instability, soliton dynamics, dispersive waves, four-wave mixing and Raman scattering, and in photonic crystal fibers it can exceed an octave${cite(10)}.</p>`,
      html5: `
        <h3>Optical parametric oscillators</h3>
        <p>In an <strong>optical parametric oscillator (OPO)</strong> the process runs the other way: inside a χ⁽²⁾ crystal each converted pump photon splits into two lower-energy photons, the <em>signal</em> and the <em>idler</em>. Naming varies: the signal often denotes the higher-frequency output${cite(13)}, or the desired one${cite(1)}; this page instead calls the resonant wave the signal. Placing the crystal in a cavity that feeds one of them back lets the parametric gain overcome the cavity losses above threshold, making a tunable coherent source for spectral ranges that direct laser emission covers poorly or not at all${cite(13, 14)}.</p>`,
      formulas5: [
      ...
      html6: `
        ...
        <p><strong>Threshold.</strong> Like a laser, an OPO oscillates only once the round-trip parametric gain overcomes the cavity losses; at threshold the two balance${cite(14, 15)}. Unlike a laser, it relies on no stored population inversion: parametric gain requires the pump to be present. For a singly resonant OPO, where only the signal is fed back, the ideal plane-wave model predicts complete pump depletion at (π/2)² ≈ 2.5 times threshold${cite(16)}; this is a theoretical limit, not a general operating point. Driven harder, signal and idler start converting back into pump light, so increasing pump power need not keep increasing the conversion efficiency${cite(15)}.</p>`,
      html7: `
        <p><strong>Linewidth.</strong> Energy conservation also holds for instantaneous frequency fluctuations, δν<sub>p</sub> = δν<sub>s</sub> + δν<sub>i</sub>, so pump fluctuations must appear on the signal, the idler or both. The cavity constrains the resonant wave, and the non-resonant wave takes up what the pump and resonant wave leave. Pump frequency fluctuations that the resonant wave does not follow appear on the non-resonant one, so imposing them on the resonant wave with active feedback is a way to remove them from the other${cite(16)}. This page and OpticalSetup call the resonant wave the signal; real OPOs resonate either. If pump and resonant wave fluctuate independently and both spectra are Gaussian, the non-resonant wave's frequency width is the quadrature sum of theirs; correlated fluctuations change that, which is why an idler can be narrower than its pump.</p>
        ...
        <ul>
          ...
          <li><strong>Nanosecond OPOs</strong> pumped by Q-switched lasers are often used for high-energy pulses. Without spectral narrowing their linewidth is typically set mainly by the crystal's phase-matching bandwidth and the cavity, and is much broader than a single-frequency laser's${cite(15)}. The oscillation needs time to build up from noise during each pump pulse, which raises the threshold${cite(15)} and often makes the output pulses somewhat shorter than the pump's${cite(14)}.</li>
    ...
    inOpticalSetup: {
      html: `<p>Choose a conversion mode before expecting any output: with the default <em>None</em> the crystal does not interact with light at all. Every converting mode is a wavelength-and-power proxy that converts a fixed, authored fraction of the eligible incident light, whatever its intensity, and <em>Transmit residual pump</em> can keep the unconverted remainder on the same path; no residual branch is drawn at efficiencies of 99.9 % or more, and in non-OPO modes it is also omitted when the output centre wavelength equals the input centre wavelength.</p>
        ...
        <p><strong>SFG and DFG</strong> mix two beams rather than acting on one, so they are the only modes that produce nothing under single-beam illumination. The crystal pairs the incident light with the longest other wavelength reaching it, at least 1 nm away, and emits once per pair: 1/λ₃ = 1/λ₁ + 1/λ₂ for sum frequency, and 1/λ₁ − 1/λ₂ with λ₁ the shorter input for difference frequency, which is therefore the longest of the three. The <em>Mixing</em> readout names the pair, the output, and how far apart the two pulses arrive.</p>
        <p><strong>Both pulses must reach the crystal together.</strong> The arrival of each beam is its own optical path plus its emission offset, so moving one source, adding glass, or scanning a delay stage shifts it. The mixed signal is scaled by exp(−(Δt/τ)²), with τ the longer of the two durations, and disappears below 2 % — which is what makes a delay scan through zero look like a cross-correlation, and is how time zero is found on a real bench. Two beams at different repetition rates produce nothing unless one rate is an exact multiple of the other; a continuous beam is always present, so it needs no timing. When a pair is present but not synchronised, the workbench says so rather than silently drawing nothing. The mixed pulse duration is (τ₁⁻² + τ₂⁻²)^(−1/2), so it follows the shorter input.</p>
        <p><strong>OPO mode</strong> models a singly resonant oscillator phenomenologically. Set the pump wavelength the crystal is phase-matched for, its acceptance window, and the resonant signal wavelength; the idler is shown as a readout. Pump light converts when its centre lies inside the acceptance window, whatever its bandwidth. The signal stays where the cavity holds it and the idler follows the arriving pump by energy conservation. Signal and idler light generated by this OPO, and its descendants, is not converted again by the same OPO; a returning residual pump may convert again, and another crystal can convert the generated light. The conversion efficiency is a fixed fraction of the pump, divided between signal and idler by the lossless Manley–Rowe split.</p>
