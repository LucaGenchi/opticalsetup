...
export const wikiEntries = [
  ...
  },
  {
    ...
    realWorld: {
      ...
      html2: `
        <p>Because the photon fluxes are equal, the longer-wavelength output is always generated with the smaller share of the power; what finally leaves a resonator also depends on its output coupling and losses. Pumped at 532 nm with an 800 nm signal, the idler lies at 1588 nm and receives about a third of the converted power. The penalty grows with the wavelength ratio: producing 5 µm light from a 1 µm pump can use at most about 20 % of the pump power${cite(3)}.</p>
      ...
      html3: `
        <p><strong>Linewidth.</strong> The frequencies must still add up, so the width of the pump spectrum has to go somewhere. The cavity holds the resonant wave, and the non-resonant wave takes up whatever the pump and resonant wave leave. Pump frequency fluctuations that the resonant wave does not follow appear on the non-resonant one, so imposing them on the resonant wave is a way to remove them from the other${cite(4)}. This page and OpticalSetup call the resonant wave the signal; real OPOs resonate either. If pump and resonant wave fluctuate independently and both spectra are Gaussian, the non-resonant wave's frequency width is the quadrature sum of theirs; correlated fluctuations change that, which is why an idler can be narrower than its pump.</p>
        <p><strong>Common operating regimes:</strong></p>
        <ul>
          <li><strong>Synchronously pumped fs and ps OPOs.</strong> The cavity round trip is matched to the repetition period of a mode-locked pump, so each signal pulse is amplified by the next pump pulse, and the output pulse trains stay locked to the pump's. One MgO:PPLN oscillator pumped by 80–100 fs pulses near 1 µm at 80 MHz produced 400–600 fs signal pulses and an idler tunable from 3132 to 4273 nm${cite(5)}. Frequency-doubled mode-locked lasers can pump picosecond OPO sources for coherent Raman imaging; near-transform-limited pulses of a few picoseconds can provide bandwidths comparable to many molecular Raman bands, balancing spectral selectivity against peak intensity${cite(6)}.</li>
          <li><strong>Nanosecond OPOs</strong> pumped by Q-switched lasers are the high-energy workhorses. Without spectral narrowing their linewidth is set mainly by the crystal's phase-matching bandwidth and the cavity, and is much broader than a single-frequency laser's${cite(3)}. The oscillation needs time to build up from noise during each pump pulse, which raises the threshold${cite(3)} and often makes the output pulses somewhat shorter than the pump's${cite(1)}.</li>
          <li><strong>Continuous-wave singly resonant OPOs</strong> pass the pump frequency fluctuations that the resonant wave does not follow on to the non-resonant wave${cite(4)}.</li>
        </ul>`,
    },
    inOpticalSetup: {
