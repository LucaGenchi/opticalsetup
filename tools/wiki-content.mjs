...
export const wikiEntries = [
...

  {
    ...
    inOpticalSetup: {
      html: `
        ...
        pump and other wavelengths. For a broadband beam, the transmitted and reflected branches
  ...
  },
  {
    ...
    realWorld: {
      ...
      html3: `
        <p><strong>Linewidth.</strong> The frequencies must still add up, so the width of the pump spectrum has to go somewhere. The cavity holds the resonant wave, and the non-resonant wave takes up whatever the pump and resonant wave leave. Pump frequency fluctuations that the resonant wave does not follow appear on the non-resonant one, so imposing them on the resonant wave is a way to remove them from the other${cite(5)}. This page and OpticalSetup call the resonant wave the signal; real OPOs resonate either. If pump and resonant wave fluctuate independently and both spectra are Gaussian, the non-resonant wave's frequency width is the quadrature sum of theirs; correlated fluctuations change that, which is why an idler can be narrower than its pump.</p>
        ...
        <ul>
          ...
          <li><strong>Nanosecond OPOs</strong> pumped by Q-switched lasers are the high-energy workhorses. Their linewidth is set mainly by the crystal's phase-matching bandwidth and the cavity rather than by the pump: a 355 nm-pumped system tunes a signal from 410 to 710 nm and an idler from 710 to 2600 nm in 3–5 ns pulses, with a linewidth below 5 cm⁻¹${cite(8)}. The oscillation needs time to build up from noise during each pump pulse, which raises the threshold${cite(4)} and makes the output pulses somewhat shorter than the pump's${cite(2)}.</li>
          <li><strong>Continuous-wave singly resonant OPOs</strong> pass the pump frequency fluctuations that the resonant wave does not follow on to the non-resonant wave${cite(5)}.</li>
        </ul>`,
    },
    inOpticalSetup: {
