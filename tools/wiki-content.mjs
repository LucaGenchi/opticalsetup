  ...
  },
  {
    ...
    inOpticalSetup: {
      ...
      html2: `
        ...
        <p><strong>Two stages.</strong> Because every beam leaves at the height it came in, a second OPA placed behind the first receives the residual pump in its pump port and the amplified signal in its seed port, as in the usual preamplifier followed by a power amplifier${cite(4, 7)}. The second stage is planned from what the first one actually delivers: its pump is the first stage's leftover pump, and its seed is the first stage's whole signal — the seed that passed through and the gain added to it — with the spectrum and timing the first stage gave them. The first stage's idler leaves on the axis and is stopped by the second stage's rear face. Up to six stages are planned in turn; the readout says if a longer chain has not settled. Light sent back into an OPA it already went through is stopped at the port rather than amplified again in a loop, and the readout says so.</p>
