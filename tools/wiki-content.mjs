  ...
  },
  {
    ...
    realWorld: {
      ...
      formulas: [
        ...
        { tex: 'G=\\cosh^2(\\Gamma L),\\quad \\Gamma\\propto\\sqrt{I_p}', caption: 'Seeded parametric power gain without phase mismatch; the gain coefficient grows with the square root of the pump intensity.' },
        { tex: '\\eta_{overlap}=\\frac{\\tau_s}{\\sqrt{\\tau_s^2+\\tau_p^2}}', caption: 'Fraction of a Gaussian pump pulse’s energy that a centred Gaussian seed overlaps in time, why the seed is stretched toward the pump duration.' },
      ],
    },
    inOpticalSetup: {
      html: `
        ...
        <p><em>Small-signal power gain</em> G₀ is the gain at full pump intensity, before saturation. A timing offset lowers the pump intensity the seed meets to a fraction f (the Gaussian temporal-overlap factor), and the gain follows cosh²(√f · arcosh √G₀), so it collapses much faster than linearly. <em>Maximum pump depletion</em> is the largest fraction of the <em>overlapped</em> pump the stage can remove; only τ<sub>s</sub>/√(τ<sub>s</sub>² + τ<sub>p</sub>²) of the pump pulse overlaps the seed, using both durations as they arrive, stretching included. The actual transfer is the smaller of that pump budget and the power the gain asks for. The generated power is divided between signal and idler by Manley–Rowe; the original seed power remains in the signal. The stage readouts report overlap, realized gain, pump depletion, the share of the pump pulse the seed reaches, and all three output powers.</p>
      ...
      limitations: `<p>This is a stage-level energy-budget model, not a nonlinear propagation solver. Gain and maximum depletion are authored; crystal material, d<sub>eff</sub>, length, beam area, fluence, phase-matching bandwidth, spatial overlap, walk-off, back-conversion, gain narrowing, higher-order spectral phase and damage are not calculated. One pump and one longer-wavelength seed are selected at a stage. Durations are Gaussian FWHM estimates; the amplified seed envelope is retained rather than reshaped or clipped by the finite pump, so gain narrowing of a seed longer than its pump is not shown. The idler duration follows the Gaussian product of the two input pulses; its chirp, which is conjugate to the seed’s for a narrowband pump, is not carried, so a compressor acting on the idler is not meaningful here. The fixed ports are a workbench packaging convention, not a prescription for a particular laser.</p>`,
    },
