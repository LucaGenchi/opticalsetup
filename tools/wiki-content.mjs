  ...
  },
  {
    ...
    realWorld: {
      ...
      html3: `
        ...
        <p><strong>Variants.</strong> A noncollinear geometry (NOPA) matches the signal and idler group velocities and amplifies bandwidths wide enough for pulses of a few femtoseconds${cite(5)}. Stretching the seed before amplification and compressing it afterwards — optical parametric chirped-pulse amplification (OPCPA) — lets a long, energetic pump pulse be converted efficiently${cite(6, 8)}. With signal and idler identical in wavelength and polarization, the amplifier becomes phase-sensitive and can amplify without excess noise; without any seed, the crystal amplifies vacuum fluctuations into optical parametric generation${cite(2)}.</p>
        <p><strong>OPA or laser amplifier?</strong> An OPA has no transition to tie it to a wavelength, so it tunes across the transparency of its crystal; its gain per unit length under pulsed pumping far exceeds a laser's; it deposits almost no heat, since the pump energy leaves as signal and idler; and it has no stored energy, so it amplifies only during the pump pulse${cite(2)}. The price is the need for an intense, well-timed pump and, at high gain and power, effects such as gain guiding that limit how far the output can be scaled${cite(9)}.</p>`,
    },
    inOpticalSetup: {
      html: `
        ...
        <ul>
          ...
          <li><em>Gain bandwidth</em> — the full width at half maximum of the gain spectrum, a Gaussian around the tuned wavelength.</li>
      ...
      formulas: [
        { tex: 'G_0(\\lambda) = 1 + \\left(10^{G_\\text{dB}/10} - 1\\right)\\, e^{-4\\ln 2\\,(\\lambda - \\lambda_0)^2/\\Delta\\lambda^2}', caption: 'The gain spectrum this element uses at the pump’s peak: a Gaussian of the authored bandwidth Δλ around the tuned wavelength λ₀. It is the one phenomenological step; the rest is computed.' },
      ],
      html2: `
