  ...
  },
  {
    ...
    inOpticalSetup: {
      html: `
        ...
        <p>The <strong>signal</strong> leaves the port marked S on the body axis, and the <strong>idler</strong> the port marked I, a fixed distance below it and parallel to it; both rotate with the body. <em>Output idler</em> switches that port off, which removes the idler's power from the bench rather than handing it to the signal. If the signal is set equal to the idler wavelength — degeneracy — the two leave together through the signal port. Each output leaves as a beam of its own <em>beam diameter</em>, whatever the pump's width, and a diameter of 0 draws it as a single line. A sized pump beam's samples keep their order across the output, so a pump clipped by the aperture loses the part that did not get in; a single-line pump is spread across the output diameter with its power shared. The ports and the body grow with the diameters so the two beams never overlap. The outputs take no path inside the box: their timing is referenced to the pump's arrival at the aperture, not to a cavity length.</p>
