  ...
  },
  {
    ...
    realWorld: {
      ...
      html2: `
        ...
        <p>For the bulk electric-dipole response, χ⁽²⁾ vanishes in any inversion-symmetric medium — a gas, a liquid, a glass or a centrosymmetric crystal — so second-order processes need a material without inversion symmetry. Surfaces, interfaces, applied fields and higher-multipole contributions fall outside that rule. χ⁽³⁾ is symmetry-allowed in centrosymmetric media as well${cite(1)}. The main processes are:</p>
        <ul>
          ...
          <li><strong>Parametric amplification and oscillation</strong>: one pump photon splits into a signal and an idler photon. In a single pass this amplifies a seed, as an optical parametric amplifier (OPA); in a resonator that feeds the signal or idler back it becomes an optical parametric oscillator (OPO), described below${cite(1)}.</li>
          <li><strong>Third-harmonic generation</strong> (THG): light at ω produces 3ω, either directly through χ⁽³⁾ or sequentially through two second-order steps, SHG to 2ω followed by sum-frequency mixing of 2ω with the remaining ω. The sequential route is often implemented with separate SHG and SFG stages${cite(3)}.</li>
        </ul>
        ...
        <p>Energy conservation is not enough. The harmonic is driven by a polarization wave that travels with the fundamental, at wavevector 2k<sub>ω</sub>, but it propagates freely with its own wavevector k<sub>2ω</sub>, and dispersion generally makes the two differ. Harmonic light generated at different depths in the crystal then adds with different phases. Over one <strong>coherence length</strong> the driven and free waves slip by π; beyond it, newly generated contributions interfere destructively with the existing harmonic, and the undepleted harmonic oscillates with crystal length instead of growing${cite(4)}. For an undepleted plane-wave fundamental in a crystal of length L, the harmonic field grows as L·d<sub>eff</sub>·sinc(ΔkL/2), with sinc(x) = sin(x)/x, so the harmonic intensity is quadratic in the fundamental intensity and falls away once |Δk|L is no longer small${cite(4)}.</p>`,
      formulas2: [
        ...
        { tex: 'I_{2\\omega} \\propto d_{\\text{eff}}^{2}\\,L^{2}\\,I_{\\omega}^{2}\\,\\operatorname{sinc}^{2}\\!\\left(\\frac{\\Delta k\\,L}{2}\\right)', caption: 'Undepleted plane-wave SHG, with sinc(x) = sin(x)/x: quadratic in the fundamental intensity and peaked at Δk = 0.' },
      ],
      html3: `
        <h3>Phase matching</h3>
        <p><strong>Phase matching</strong> makes Δk vanish. In <strong>birefringent phase matching</strong> the interacting waves travel with different polarizations, so the dispersion between fundamental and harmonic is offset by the difference between ordinary and extraordinary refractive indices; Giordmaine, and Maker and colleagues, demonstrated it in 1962${cite(5, 6)}. It is often tuned by the angle between the beam and the crystal's optic axis. For general propagation directions in a birefringent medium, an extraordinary wave's energy flow is not parallel to its wavevector, so the beams drift apart. This <strong>spatial walk-off</strong> can reduce beam overlap and limit the useful interaction length; suitable principal-axis geometries avoid it${cite(4, 7)}.</p>
        <p>In the sinc² law above the tolerable phase mismatch scales as 1/L: a longer crystal converts more strongly but tolerates a smaller Δk. Acceptance in wavelength, angle or temperature inherits that 1/L scaling where Δk varies linearly with the tuning parameter near the operating point${cite(8)}. Where that first derivative vanishes, Δk is quadratic in the detuning and the acceptance scales as L<sup>−1/2</sup> instead.</p>
        ...
        <p><strong>Quasi-phase matching</strong> takes a different route. Instead of matching phase velocities, the sign of the nonlinear coefficient is periodically reversed so that the relative phase is reset before new contributions start to cancel the harmonic; for the simplest first-order grating with a 50 % duty cycle, the reversal comes every coherence length. It was proposed by Armstrong, Bloembergen and colleagues in 1962 and became widely practical once patterned poling of ferroelectrics such as lithium niobate developed from the late 1980s. Because it does not require birefringent phase matching, all waves can share one polarization and use the largest tensor component, propagation along a crystal axis can avoid birefringent spatial walk-off, and non-birefringent materials such as GaAs can be used. The price for a first-order grating is an effective coefficient of at most 2/π of d<sub>eff</sub>${cite(4, 9)}.</p>`,
      formulas3: [
        { tex: '\\Delta k_{\\text{QPM}} = k_{2\\omega} - 2k_{\\omega} - \\frac{2\\pi}{\\Lambda}, \\qquad |d_1| = \\frac{2}{\\pi}\\,|d_{\\text{eff}}|\\,\\sin(\\pi D)', caption: 'First-order quasi-phase matching with a grating of period Λ and duty cycle D (0 ≤ D ≤ 1): matched when Δk_QPM = 0, so that Λ = 2ℓ_c, with the effective coefficient |d_1| largest at D = ½.' },
      ],
      html4: `
        ...
        <p>A real conversion stage is therefore designed around more than one phase-matching condition: the wavelengths involved and the crystal's transparency and absorption there, the allowed polarizations with the crystal cut or poling period, the operating temperature, the crystal length and focusing, the pulse duration, and coating and optical-damage limits. d<sub>eff</sub>, the acceptance bandwidths and the walk-off all follow from those choices.</p>
        <h3>Supercontinuum</h3>
        <p><strong>Supercontinuum generation</strong> turns intense pulses into a broad continuum, often spanning hundreds of nanometres, rather than a new discrete line. Early demonstrations came in 1970, when Alfano and Shapiro broadened picosecond pulses in glasses and crystals${cite(10, 11)}. In glass and optical fibers the broadening arises mainly from third-order effects: depending on pump duration and dispersion it can involve self-phase modulation, modulation instability, soliton dynamics, dispersive waves, four-wave mixing and Raman scattering, and in photonic crystal fibers it can exceed an octave${cite(12)}.</p>`,
      html5: `
        <h3>Optical parametric oscillators</h3>
        <p>In an <strong>optical parametric oscillator (OPO)</strong> the process runs the other way: inside a χ⁽²⁾ crystal each pump photon splits into two lower-energy photons, the <em>signal</em> and the <em>idler</em>. By convention the signal is the shorter of the two. Placing the crystal in a cavity that feeds one of them back turns this weak parametric gain into a tunable coherent source for spectral ranges that direct laser emission covers poorly or not at all${cite(13, 14)}.</p>`,
      formulas5: [
      ...
      html6: `
        <p>Because the photon fluxes are equal, the longer-wavelength output is always generated with the smaller share of the power; what finally leaves a resonator also depends on its output coupling and losses. Pumped at 532 nm with an 800 nm signal, the idler lies at 1588 nm and receives about a third of the converted power. The penalty grows with the wavelength ratio: producing 5 µm light from a 1 µm pump can use at most about 20 % of the pump power${cite(15)}.</p>
      ...
      html7: `
        <p><strong>Linewidth.</strong> The frequencies must still add up, so the width of the pump spectrum has to go somewhere. The cavity holds the resonant wave, and the non-resonant wave takes up whatever the pump and resonant wave leave. Pump frequency fluctuations that the resonant wave does not follow appear on the non-resonant one, so imposing them on the resonant wave is a way to remove them from the other${cite(16)}. This page and OpticalSetup call the resonant wave the signal; real OPOs resonate either. If pump and resonant wave fluctuate independently and both spectra are Gaussian, the non-resonant wave's frequency width is the quadrature sum of theirs; correlated fluctuations change that, which is why an idler can be narrower than its pump.</p>
        <p><strong>Common operating regimes:</strong></p>
        <ul>
          <li><strong>Synchronously pumped fs and ps OPOs.</strong> The cavity round trip is matched to the repetition period of a mode-locked pump, so each signal pulse is amplified by the next pump pulse, and the output pulse trains stay locked to the pump's. One MgO:PPLN oscillator pumped by 80–100 fs pulses near 1 µm at 80 MHz produced 400–600 fs signal pulses and an idler tunable from 3132 to 4273 nm${cite(17)}. Frequency-doubled mode-locked lasers can pump picosecond OPO sources for coherent Raman imaging; near-transform-limited pulses of a few picoseconds can provide bandwidths comparable to many molecular Raman bands, balancing spectral selectivity against peak intensity${cite(18)}.</li>
          <li><strong>Nanosecond OPOs</strong> pumped by Q-switched lasers are the high-energy workhorses. Without spectral narrowing their linewidth is set mainly by the crystal's phase-matching bandwidth and the cavity, and is much broader than a single-frequency laser's${cite(15)}. The oscillation needs time to build up from noise during each pump pulse, which raises the threshold${cite(15)} and often makes the output pulses somewhat shorter than the pump's${cite(13)}.</li>
          <li><strong>Continuous-wave singly resonant OPOs</strong> pass the pump frequency fluctuations that the resonant wave does not follow on to the non-resonant wave${cite(16)}.</li>
        </ul>`,
    },
    inOpticalSetup: {
      html: `<p>Choose a conversion mode before expecting any output: with the default <em>None</em> the crystal does not interact with light at all. Every mode is a wavelength-and-power proxy that converts a fixed, authored fraction of the eligible incident light, whatever its intensity, and <em>Transmit residual pump</em> keeps the unconverted remainder on the same path.</p>
        <p><strong>SHG and THG</strong> divide the wavelength of any incident light by two or three: there is no phase-matching condition, so every input wavelength converts. THG maps a wavelength directly to a third of it; it does not simulate a cascaded SHG and SFG apparatus. The spectrum is scaled with the wavelength and the pulse keeps its train and duration, which makes the harmonic's frequency width n times the input's, where the undepleted, ideal Gaussian limit would give √n with a √n shorter pulse. The example doubles a 1064 nm pulsed laser to 532 nm and separates the harmonic from the remaining pump with a dichroic, with a beam probe on each branch.</p>
        <p><strong>Supercontinuum</strong> replaces the converted light with a fixed, flat 430–870 nm band, whatever the pump's wavelength or power: it stands in for a continuum source rather than modelling its generation. <strong>Custom output</strong> is an authored output rather than a named physical process: it emits a single line at the entered wavelength, whatever the pump's bandwidth.</p>
