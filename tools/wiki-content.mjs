...
export const wikiEntries = [
  ...
  },
  {
    ...
    realWorld: {
      html: `
        <p>A <strong>nonlinear optical crystal</strong> responds to intense light with a polarization that is no longer simply proportional to the optical field. Expanding the induced polarization in powers of the field separates the effects by order: the linear susceptibility χ⁽¹⁾ gives the ordinary refractive index, the second-order susceptibility χ⁽²⁾ mixes pairs of fields, and the third-order χ⁽³⁾ mixes three${cite(1)}. Each higher order is a far smaller correction at the fields of ordinary light, which is why nonlinear optics had to wait for the laser: in 1961 Franken and colleagues focused a pulsed ruby laser into crystalline quartz and detected its second harmonic${cite(2)}.</p>`,
      formulas: [
        { tex: 'P = \\varepsilon_0\\left(\\chi^{(1)}E + \\chi^{(2)}E^{2} + \\chi^{(3)}E^{3} + \\dots\\right)', caption: 'The induced polarization expanded in powers of the optical field E.' },
      ],
      html2: `
        <h3>Second-order processes and phase matching</h3>
        <p>A second-order response requires a material without inversion symmetry: in a centrosymmetric medium — a gas, a liquid, a glass, or a centrosymmetric crystal — χ⁽²⁾ vanishes, whereas χ⁽³⁾ is present in every material${cite(1)}. χ⁽²⁾ couples three waves whose frequencies add up. Two photons at ω combine into one at 2ω in <strong>second-harmonic generation</strong> (SHG); two different frequencies add or subtract in sum- and difference-frequency generation; and in the reverse process one photon splits into two, which is parametric amplification and oscillation (below). The strength of the coupling is an effective coefficient d<sub>eff</sub>, a combination of χ⁽²⁾ tensor components set by the crystal, the propagation direction and the polarizations${cite(1)}.</p>
        <p>Energy conservation is not enough. The harmonic is driven by a polarization wave that travels with the fundamental, at wavevector 2k<sub>ω</sub>, but it propagates freely with its own wavevector k<sub>2ω</sub>, and normal dispersion makes the two differ. After one <strong>coherence length</strong> the driven and free waves are out of step by π, power starts flowing back into the fundamental, and the harmonic oscillates along the crystal instead of growing${cite(3)}. For an undepleted plane-wave fundamental in a crystal of length L, the harmonic field grows as L·d<sub>eff</sub>·sinc(ΔkL/2), so the harmonic intensity rises with the square of the fundamental intensity and collapses once ΔkL is no longer small${cite(3)}.</p>`,
      formulas2: [
        { tex: '\\Delta k = k_{2\\omega} - 2k_{\\omega}, \\qquad \\ell_c = \\frac{\\pi}{\\Delta k} = \\frac{\\lambda}{4\\,(n_{2\\omega} - n_{\\omega})}', caption: 'Phase mismatch and coherence length for SHG, with λ the fundamental vacuum wavelength.' },
        { tex: 'I_{2\\omega} \\propto d_{\\text{eff}}^{2}\\,L^{2}\\,I_{\\omega}^{2}\\,\\operatorname{sinc}^{2}\\!\\left(\\frac{\\Delta k\\,L}{2}\\right)', caption: 'Undepleted plane-wave SHG: quadratic in the fundamental intensity and peaked at Δk = 0.' },
      ],
      html3: `
        <p><strong>Phase matching</strong> makes Δk vanish. In <strong>birefringent phase matching</strong> the interacting waves travel with different polarizations, so the extra dispersion at the harmonic is offset by the difference between ordinary and extraordinary refractive indices; Giordmaine, and Maker and colleagues, demonstrated it in 1962${cite(4, 5)}. It is usually tuned by the angle between the beam and the crystal axis. When that angle is not 90°, the extraordinary beam's energy flow walks off from the ordinary beam's, which limits the useful overlap length${cite(3)}. Because conversion follows sinc²(ΔkL/2), the tolerable mismatch scales as 1/L: a longer crystal converts more strongly but accepts a narrower range of wavelengths and angles${cite(3)}.</p>
        <p><strong>Quasi-phase matching</strong> takes a different route. Instead of matching phase velocities, the sign of the nonlinear coefficient is reversed every coherence length, resetting the relative phase before power can flow back. It was proposed by Armstrong, Bloembergen and colleagues in 1962 and became widely practical once lithographically patterned poling of ferroelectrics such as lithium niobate developed from the late 1980s. Because it does not rely on birefringence, all waves can share one polarization and use the largest tensor component, propagation can run along a crystal axis without walk-off, and even non-birefringent materials such as GaAs can be used; the price is an effective coefficient reduced by 2/π for a first-order grating${cite(3, 6)}.</p>`,
      formulas3: [
        { tex: '\\Delta k_{\\text{QPM}} = k_{2\\omega} - 2k_{\\omega} - \\frac{2\\pi m}{\\Lambda}, \\qquad d_m = d_{\\text{eff}}\\,\\frac{2\\sin(\\pi D)}{m\\pi}', caption: 'Quasi-phase matching with a grating of period Λ, order m and duty cycle D: matched when Δk_QPM = 0, with effective coefficient d_m, which is (2/π)·d_eff for m = 1 and D = ½.' },
      ],
      html4: `
        <h3>Third harmonic and supercontinuum</h3>
        <p>The third harmonic can be generated directly through χ⁽³⁾, which every material possesses, but that term is a much smaller correction than χ⁽²⁾ at the same field. Third-harmonic generation from a laser is therefore commonly done by cascading two second-order steps: SHG produces 2ω, and sum-frequency generation of that light with the remaining fundamental produces 3ω, each step phase-matched in its own crystal${cite(1)}.</p>
        <p><strong>Supercontinuum generation</strong> is a third-order effect of a different kind: intense pulses broaden into a continuum spanning hundreds of nanometres rather than producing a new line. Alfano and Shapiro first reported it in 1970, from picosecond pulses in glass${cite(7)}. Self-phase modulation — the intensity-dependent refractive index acting on a pulse's own temporal profile — starts the broadening, and in optical fibers, above all photonic crystal fibers, soliton dynamics, dispersive-wave generation, four-wave mixing and Raman scattering can extend it beyond an octave${cite(8)}.</p>`,
      html5: `
        <h3>Optical parametric oscillators</h3>
        <p>In an <strong>optical parametric oscillator (OPO)</strong> the process runs the other way: inside a χ⁽²⁾ crystal each pump photon splits into two lower-energy photons, the <em>signal</em> and the <em>idler</em>. By convention the signal is the shorter of the two. Placing the crystal in a cavity that feeds one of them back turns this weak parametric gain into a tunable coherent source for spectral ranges that direct laser emission covers poorly or not at all${cite(9, 10)}.</p>`,
      formulas5: [
        { tex: '\\frac{1}{\\lambda_p} = \\frac{1}{\\lambda_s} + \\frac{1}{\\lambda_i}', caption: 'Energy conservation: one pump photon becomes one signal and one idler photon.' },
      ...
      html6: `
        <p>Because the photon fluxes are equal, the longer-wavelength output is always generated with the smaller share of the power; what finally leaves a resonator also depends on its output coupling and losses. Pumped at 532 nm with an 800 nm signal, the idler lies at 1588 nm and receives about a third of the converted power. The penalty grows with the wavelength ratio: producing 5 µm light from a 1 µm pump can use at most about 20 % of the pump power${cite(11)}.</p>
        <p><strong>Threshold.</strong> Like a laser, an OPO oscillates only once the round-trip parametric gain exceeds the cavity losses. Unlike a laser, the crystal stores no energy: gain exists only while the pump is present. In a singly resonant OPO, where only the signal is fed back, plane-wave theory predicts complete pump depletion at (π/2)² ≈ 2.5 times threshold. Driven harder, signal and idler start converting back into pump light, which is why the efficiency saturates${cite(11)}.</p>`,
      formulas6: [
        { tex: '\\frac{x}{\\sin x} = \\sqrt{N}, \\qquad D = \\sin^2 x, \\qquad N = \\frac{P_p}{P_{\\text{th}}}', caption: 'Pump depletion D of a plane-wave singly resonant OPO at N times threshold: zero at N = 1, complete at N = (π/2)².' },
      ],
      html7: `
        <p><strong>Linewidth.</strong> The frequencies must still add up, so the width of the pump spectrum has to go somewhere. The cavity holds the resonant wave, and the non-resonant wave takes up whatever the pump and resonant wave leave. Pump frequency fluctuations that the resonant wave does not follow appear on the non-resonant one, so imposing them on the resonant wave is a way to remove them from the other${cite(12)}. This page and OpticalSetup call the resonant wave the signal; real OPOs resonate either. If pump and resonant wave fluctuate independently and both spectra are Gaussian, the non-resonant wave's frequency width is the quadrature sum of theirs; correlated fluctuations change that, which is why an idler can be narrower than its pump.</p>
        <p><strong>Common operating regimes:</strong></p>
        <ul>
          <li><strong>Synchronously pumped fs and ps OPOs.</strong> The cavity round trip is matched to the repetition period of a mode-locked pump, so each signal pulse is amplified by the next pump pulse, and the output pulse trains stay locked to the pump's. One MgO:PPLN oscillator pumped by 80–100 fs pulses near 1 µm at 80 MHz produced 400–600 fs signal pulses and an idler tunable from 3132 to 4273 nm${cite(13)}. Frequency-doubled mode-locked lasers can pump picosecond OPO sources for coherent Raman imaging; near-transform-limited pulses of a few picoseconds can provide bandwidths comparable to many molecular Raman bands, balancing spectral selectivity against peak intensity${cite(14)}.</li>
          <li><strong>Nanosecond OPOs</strong> pumped by Q-switched lasers are the high-energy workhorses. Without spectral narrowing their linewidth is set mainly by the crystal's phase-matching bandwidth and the cavity, and is much broader than a single-frequency laser's${cite(11)}. The oscillation needs time to build up from noise during each pump pulse, which raises the threshold${cite(11)} and often makes the output pulses somewhat shorter than the pump's${cite(9)}.</li>
          <li><strong>Continuous-wave singly resonant OPOs</strong> pass the pump frequency fluctuations that the resonant wave does not follow on to the non-resonant wave${cite(12)}.</li>
        </ul>`,
    },
    inOpticalSetup: {
      html: `<p>Choose a conversion mode before expecting any output: with the default <em>None</em> the crystal does not interact with light at all. Every mode is a wavelength-and-power proxy that converts a fixed, authored fraction of the light reaching the crystal, whatever its intensity, and <em>Transmit residual pump</em> keeps the unconverted remainder on the same path.</p>
        <p><strong>SHG and THG</strong> divide the wavelength of any incident light by two or three: there is no phase-matching condition, so every input wavelength converts. The spectrum is scaled with the wavelength and the pulse keeps its train and duration, which makes the harmonic's frequency width n times the input's, where the undepleted, ideal Gaussian limit would give √n with a √n shorter pulse. The example doubles a 1064 nm pulsed laser to 532 nm and separates the harmonic from the remaining pump with a dichroic, with a beam probe on each branch.</p>
        <p><strong>Supercontinuum</strong> replaces the converted light with a fixed, flat 430–870 nm band, whatever the pump's wavelength or power: it stands in for a continuum source rather than modelling its generation. <strong>Custom output</strong> emits a single line at the entered wavelength, whatever the pump's bandwidth.</p>
        <p><strong>OPO mode</strong> models a singly resonant oscillator phenomenologically. Set the pump wavelength the crystal is phase-matched for, its acceptance window, and the resonant signal wavelength; the idler is shown as a readout. Pump light converts when its centre lies inside the acceptance window, whatever its bandwidth. The signal stays where the cavity holds it and the idler follows the arriving pump by energy conservation. Light is never converted twice by the same crystal, even after passing through another one; a different crystal can still convert it. The conversion efficiency is a fixed fraction of the pump, divided between signal and idler by the lossless Manley–Rowe split.</p>
        <p><strong>Linewidths</strong> are handled as FWHM in wavenumber. <em>Signal as wide as the pump</em> is a heuristic for synchronously pumped fs and ps OPOs; <em>Signal width set</em> suits ns and CW OPOs, where the cavity sets it; in both, the idler is derived as the uncorrelated Gaussian sum. <em>Signal and idler widths set</em> takes both from a measured or specified system. A narrow output is a Gaussian in wavelength; one wider than 1 % of its frequency is sampled from its Gaussian in wavenumber, so it is correctly skewed toward long wavelengths. Dichroics, filters and spectrometers downstream act on these new spectra rather than on the pump's. At exactly twice the pump wavelength, signal and idler with equal widths form one degenerate beam; with different widths they stay two coincident beams, each with its own spectrum.</p>
        <p><strong>Pulses.</strong> Signal and idler are pulse trains of their own, synchronised to the pump: they keep its repetition rate, arrival timing and modulation gates, so a detector sees a non-degenerate pump, signal and idler as three separate trains. Their duration is the pump duration times an authored factor, raised to the transform limit of the output's Gaussian spectrum if that is longer, and their spectral phase is treated as unknown. Choose <em>Transform-limited</em> to set each duration from its bandwidth instead; an output with zero linewidth is a drawing convention for an idealised monochromatic pulse train, like a pulsed source set to 0 nm: it has no finite transform-limited duration, so it keeps its authored one. Group delay dispersion is counted from the crystal exit. The inspector lists each output's linewidth and duration.</p>`,
      limitations: `<p>No mode calculates phase matching, d<sub>eff</sub>, crystal length, acceptance bandwidths, walk-off, or the dependence of conversion on intensity: SHG and THG convert every wavelength at the authored fraction, and no crystal material is selected. Supercontinuum is a fixed band, not a model of self-phase modulation or soliton dynamics. Harmonic spectra are scaled rather than calculated from the field, as described above.</p><p>OPO mode is a phenomenological model rather than a cavity simulation. Phase matching is not calculated from material data: the signal wavelength and acceptance window are authored, and crystal choice or temperature do not affect them. There is no threshold, gain or pump depletion: the authored efficiency converts the same fraction at any pump power. Ray round trips and authored output-coupler losses are traced; resonant gain, synchronisation-dependent conversion, build-up time, group-velocity walk-off and spatial mode overlap are not calculated. Output durations are authored rather than predicted and carry no spectral phase. Pump spectra are reduced to a Gaussian of the same FWHM, and the idler width assumes uncorrelated Gaussian fluctuations unless both widths are set. When pump, signal and idler reach the same detector, its single pulse summary is taken from one of the trains, so read each output on its own detector.</p>`,
    },
    citations: [
      ...
      { label: 'J.-M. Melkonian, J.-B. Dherbecourt, M. Raybaut, A. Godard, “Optical Parametric Oscillators,” Photoniques 110, 53–57 (2021)', url: 'https://doi.org/10.1051/photon/202111053' },
      ...
      { label: 'A. Ly, B. Szymanski, F. Bretenaker, “Frequency stabilization of the non resonant wave of a continuous-wave singly resonant optical parametric oscillator,” Applied Physics B (2015)', url: 'https://doi.org/10.1007/s00340-015-6122-0' },
