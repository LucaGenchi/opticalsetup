...
export const wikiEntries = [
  ...
  },
  {
    ...
    realWorld: {
      ...
      html2: `
        <p>Because the photon fluxes are equal, the longer-wavelength output is always generated with the smaller share of the power; what finally leaves a resonator also depends on its output coupling and losses. Pumped at 532 nm with an 800 nm signal, the idler lies at 1588 nm and receives about a third of the converted power. The penalty grows with the wavelength ratio: producing 5 µm light from a 1 µm pump can use at most about 20 % of the pump power${cite(4)}.</p>
    ...
    inOpticalSetup: {
      html: `<p>Choose a conversion mode before expecting an output: the default <em>None</em> passes light unchanged. SHG and THG set the wavelength to one half or one third of the input. Custom output uses the entered wavelength; supercontinuum creates a fixed illustrative band. Conversion efficiency sets the converted ray weight, and residual-pump transmission retains the unconverted branch. The example doubles 1064 nm to 532 nm and separates it from the remaining pump with a dichroic.</p>
        <p><strong>OPO mode</strong> models a singly resonant oscillator phenomenologically. Set the pump wavelength the crystal is phase-matched for, its acceptance window, and the resonant signal wavelength; the idler is shown as a readout. Pump light converts when its centre lies inside the acceptance window, whatever its bandwidth. The signal stays where the cavity holds it and the idler follows the arriving pump by energy conservation. Light a crystal generated is never converted by that crystal again; another crystal can still convert it. The converted power is divided by the lossless Manley–Rowe split, and a signal at exactly twice the pump wavelength emits one degenerate beam carrying both outputs' spectra.</p>
        <p><strong>Linewidths</strong> are handled as FWHM in wavenumber. <em>Signal as wide as the pump</em> is a heuristic for synchronously pumped fs and ps OPOs; <em>Signal width set</em> suits ns and CW OPOs, where the cavity sets it; in both, the idler is derived as the uncorrelated Gaussian sum. <em>Signal and idler widths set</em> takes both from a datasheet. A narrow output is a Gaussian in wavelength; one wider than 1 % of its frequency is sampled from its Gaussian in wavenumber, so it is correctly skewed toward long wavelengths. Dichroics, filters and spectrometers downstream act on these new spectra rather than on the pump's.</p>
        <p><strong>Pulses.</strong> Signal and idler are pulse trains of their own, synchronised to the pump: they keep its repetition rate, timing and modulation gates, so a detector sees pump, signal and idler as three separate trains. Their duration is the pump duration times an authored factor, raised to the transform limit of the output's bandwidth if that is longer, and their spectral phase is treated as unknown. Choose <em>Transform-limited</em> to set each duration from its bandwidth instead. Group delay dispersion is counted from the crystal exit.</p>
        <p><strong>Threshold.</strong> With the threshold left at 0, OPO mode converts the fixed efficiency fraction, as older saved setups expect. Enter a threshold in average pump watts, or in pump pulse energy for ns OPOs, and the workbench first traces the scene to sum the pump reaching the crystal over every path, including clipping, splitting and attenuation on the way. Below threshold nothing oscillates and the pump passes. Above it, the converted fraction is the efficiency, read as the measured maximum, multiplied by the plane-wave depletion shown above and held at its maximum beyond (π/2)² times threshold. The inspector reports the pump level against threshold, the depletion, and each output's linewidth and duration.</p>`,
      limitations: `<p>Conversion is a wavelength-and-power proxy, and OPO mode is a phenomenological model rather than a cavity simulation. Phase matching is not calculated from material data: the signal wavelength and acceptance window are authored, and crystal choice or temperature do not affect them. Cavity round trips, synchronisation to the repetition rate, build-up time, output coupling, group-velocity walk-off, spatial mode overlap and back-conversion are not traced; the plateau above full depletion is an empirical choice. Output durations are authored, not predicted, and carry no spectral phase. Pump spectra are reduced to a Gaussian of the same FWHM. Pump light that reaches the crystal through a fiber is not counted towards a threshold, and the pulsed source's lowest repetition rate is 1 kHz.</p>`,
    },
    citations: [
