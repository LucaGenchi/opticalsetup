...
export const exampleEntries = [
  ...
  },
  {
    ...
    inOpticalSetupTitle: 'What this setup demonstrates',
    inOpticalSetupHtml: `
      <p>One layout, three ways to put phase into an arm. Each row is the same
      Mach–Zehnder diamond — a laser, two beamsplitters, a mirror folding each
      of the two separate arms — and each recombines onto two cameras with
      their own screens. Both arms are genuinely separate ray paths: move a
      mirror on one and only that arm's traced path changes, exactly as on a
      bench.</p>
      <p>The invariant to watch across all three is that the ports are
      complementary. Whatever leaves one arrives at the other, and they always
      sum to the input — the light is redirected, never absorbed.</p>
      <p><strong>Row 1, a delay line.</strong> Lengthen one arm mechanically.
      Equal arms make port 1 bright and port 2 dark; entering half a wavelength
      (0.000266&nbsp;mm at 532&nbsp;nm) swaps them, and a full wavelength brings
      them back. This is the interferometer as a ruler for optical path, and
      sweeping the delay traces out the cosine fringe between the two states.</p>
      <p><strong>Row 2, a phase modulator.</strong> The same swap driven by a
      voltage rather than a stage: 180° of drive depth does what half a
      wavelength of delay did. It ships already driven — half a wave of sine at
      1 MHz — so the two screens trade the light back and forth on their own,
      and the time scale is chosen on load to make one period take about a
      second. This is a Mach–Zehnder modulator, which is what almost all
      high-speed optical telecommunications runs on.</p>
      <p><strong>Row 3, a phase object.</strong> A half-wave bar across the
      middle third of the beam. The arms now disagree by different amounts at
      different heights, so the output is not a level but a <em>pattern</em>:
      the camera profile shows bright edges with a dark centre, which is the bar
      imaged. Port 1 reads 0.67 and port 2 reads 0.33 — the two thirds of the
      beam the bar misses, and the third it covers. That is phase contrast, and
      the object itself absorbs nothing: put a detector after it on its own and
      the reading is unchanged.</p>
      <p>Worth trying on all three: give the laser a coherence length. It
      defaults to zero, meaning idealised, and interferes at any arm mismatch.
      Set a real value and sweep the delay line, and the fringes fade out where
      the arms are mismatched by more than that distance.</p>`,
