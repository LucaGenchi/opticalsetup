...
<p>These are orders of magnitude, not design values: the coefficients depend on wavelength (they fall towards the infrared), on the crystal’s composition and on the measurement: Eckardt et al. give about 4 % reproducibility and about 10 % absolute accuracy ${cite(10)}. Use the d<sub>eff</sub> of your crystal cut from its data sheet or a crystal handbook ${cite(8)}.</p>`,
      },
      {
        ...
        rows: [
          ...
          ['η<sub>max</sub>', 'Depletion limit', '—', `Largest fraction of the pump energy converted in any instant. 1 is the plane-wave limit, reached at particular interaction lengths under ideal phase-matched conditions. Measured amplifiers convert less, because the wings of real beams and pulses are pumped weakly: about 35 % pump depletion is typical of chirped-pulse parametric amplifiers ${cite(22)}; a 13 J amplifier with a flat-top pump in space and time put up to 41 % of the pump energy into the signal alone ${cite(23)}; and a variant that absorbs the idler to stop back-conversion reached 85 % pump depletion, 56 % into the signal ${cite(22)}. The page starts at 0.5 and keeps the range 0–1 because 0.5 is not a universal physical ceiling. These measurements are context, not something this calculator validates or reproduces: idler absorption and flat-top beam and pulse profiles are not modelled. The limit applies to each instant: averaged over Gaussian pulses the total conversion is lower.`],
