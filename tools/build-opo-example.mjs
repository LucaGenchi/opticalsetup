...
//
// The cavity follows the femtosecond OPO of O'Donnell, Chaitanya Kumar and
// Ebrahim-Zadeh (APL Photonics 4, 050801, 2019): a MgO:PPLN crystal between
// two r = 100 mm concave mirrors M1 and M2, a plane mirror M3 and a plane
// output coupler M4 transmitting 5 % of the signal, pumped by 80–100 fs pulses
// near 1 µm at 80 MHz and emitting a 3.1–4.3 µm idler 140–180 nm wide.
//
...
// period: L = c / (2 f_rep) = 1873.70 mm for 80 MHz. The crystal is a thin
// surface here, so the whole length is air path, and it is laid out in a Z to
// fit the page: M3 → M1 → crystal → M2 → M5 → M4.
//
...
//  - The pump is a single axial ray, so the focusing lens and plane mirrors
//    act on it exactly; a finite beam would need the concave mirrors.
...
export function opoExampleScene() {
  return {
    ...
    elements: [
      ...
      text('cavity-note', 560, 118, `Cavity length ${CAVITY_LENGTH_MM.toFixed(1)} mm: one round trip = c / ${REP_RATE_MHZ} MHz,\nso each signal pulse returns to the crystal with the next pump pulse`, 10),
      ...
      el('crystal', 'crystal', CRYSTAL, {
        ...
        linewidthMode: 'both', signalLinewidthCm: 147, idlerLinewidthCm: 131,
        outputPhase: 'unknown', durationFactor: 1, efficiency: 0.78, transmitPump: true,
      }, named('MgO:PPLN')),
      ...
      text('legend', 560, 590, 'Only the 1480 nm signal resonates. The crystal converts a fixed 78 % of the pump,\nthe reported maximum pump depletion: threshold and gain are not modelled.', 10),
    ],
  };
}
