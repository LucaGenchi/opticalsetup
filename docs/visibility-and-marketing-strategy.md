...
**Last verified:** 2026-07-29 — reviewed and updated 2026-08-20 against the shipped feature set  
...
> **2026-08-20 review note:** the strategy, staged launch, audience map, and outreach mechanics below still hold. Two things changed materially since this was drafted and are folded in throughout: (1) a **wiki encyclopedia** now exists at `/wiki/` — 24 illustrated component pages (of 58 library components, ~41% coverage) each pairing real-world physics/formulas/citations with an honest "In OpticalSetup" section and a live embedded demo — and (2) the **community gallery** described below as a future asset is already live at `/community/` with 5 published setups. Both change the "assets to create first" and "high-priority pages" sections from a from-scratch build to a coverage/promotion problem. See the inline notes below for what is done, what is genuinely still missing, and what a nearly-merged GIF export feature unlocks.
...
- a growing **wiki encyclopedia** (`/wiki/`) pairing real-world physics with exactly how each component is simplified — a genuine long-tail SEO asset, not just a feature list;
- publication-ready SVG/PNG export;
- self-contained share links and QR codes;
- a live **community gallery** (`/community/`) of real user-submitted setups, each with its own permanent page and embed;
...
- **Six canonical demo setups** with permanent URLs. The "permanent URL" part is
  the actual gap: several of these already have the *content* built, but only
  `/community/<slug>/` setups have an individual, indexable page today — a
  built-in Example is just a JSON file loaded through the in-app dropdown, with
  no page, URL, or metadata of its own yet.
  - microscope — **content exists**: the "Bright field microscope" community
    setup already has a permanent URL (`/community/bright-field-microscope/`).
    (Note: this is a compound scene built from the Objective, Sample, and
    detector components, not a single all-in-one "microscope" element — that
    standalone element was removed from the library in favor of composing the
    real parts.)
  - Mach–Zehnder interferometer — **content exists, no page yet**: it's a
    built-in Example with no dedicated URL. Either route it through the
    community pipeline or build the missing per-Example page.
  - grating spectrometer — **content likely exists**: the "Optical
    spectrometer" community setup already has a permanent URL; confirm it's
    grating-based and promote it, or publish a dedicated one if not.
  - fluorescence microscopy path — **still missing entirely**. The underlying
    physics now supports it well (fluorophore-specific absorption/emission
    bands, epi-illumination, multi-photon excitation), so this is now a content
    task, not an engineering one.
  - pulsed-laser delay line — **partially covered**, content-wise, by the
    built-in "OPTICAL SETUP — pulsed component panorama" example, but that's a
    broad showcase, not a focused delay-line demo, and it has no dedicated URL
    either. Worth a narrower, purpose-built community submission.
  - telescope or camera image formation — **still missing** as packaged content
    (the README's example list mentions these, but they are not yet in the
    actual Examples menu — verify before promising this in outreach).
...
- ~~Embeddable, read-only setup viewer.~~ **Done.** Every wiki page embeds a
  locked, click-to-inspect live canvas (`/sketch/?demo=<type>`), and every
  community submission gets the same treatment
  (`/sketch/?community=<slug>`). Nothing left to build here — just make sure
  outreach material actually links to these embeds instead of describing the
  idea as future work.
- A downloadable classroom handout with three exercises.
- A public gallery with filters for education, research, microscopy, spectroscopy, interferometry, lasers, and imaging — **partially done**: `/community/` is a live gallery of real submissions, but it has no category filters yet and only 5 entries. Filters are worth adding once the count grows; for now the more useful investment is recruiting more submissions (see Section 8's student-chapter program).
- Two short testimonials from real researchers or instructors, with permission.
- A 20–30 second looping GIF for social posts and press emails — **about to become trivial**: an in-app "Export GIF" feature (bounded duration/fps/size, deterministic pulsed/mechanical playback capture) is queued for merge. Once it ships, this asset is a two-minute in-app export rather than a separate production task, and it's worth adding directly to the Phase 0 checklist below.
...
The current landing page already communicates free browser access, live ray tracing, export, sharing, and honest simulation scope, and already links to both `/wiki/` and `/community/` in its header. The next visibility gains will come from **specific entry pages**, not from making the homepage longer — and a meaningful share of that work is already done via the wiki, not still ahead of us.

### The wiki now covers most of the component-level search intent

`/wiki/` is a real long-tail SEO asset today: 24 pages (of 58 library components), each with a unique title/description/canonical URL/OG image, a "real world" physics section with formulas and citations, an honest "In OpticalSetup" simplification section, related-component cross-links, and a live embedded demo — i.e. it already satisfies most of the bullet list below for the components it covers. **The highest-leverage SEO work right now is closing that coverage gap** (34 components still without a page) rather than inventing a new page type. Prioritize components people are likely to search for by name (objective, polarizer, beamsplitter-family) over obscure ones.

### High-priority pages

Two distinct kinds of page remain worth creating, and they should not be confused with the wiki:

**Tool-category pages** (broad, non-component search intent — these have no existing equivalent):
...
**Named-setup pages** (Michelson, Mach–Zehnder, fluorescence microscope, grating spectrometer, laser cavity, telescope ray diagram) — do not build a brand-new `/examples/<slug>/` URL namespace for these from scratch. The `/community/<slug>/` page template already does almost everything this section asks for — embedded locked canvas, permanent URL, its own title/description — for setups that go through the community pipeline. **Built-in Examples do not have this yet** (they're just JSON files loaded through the in-app dropdown, with no individual page, URL, or metadata) — that's a real, still-open gap. The fastest path to all six canonical demos is likely routing the missing ones through the community pipeline (which already works) rather than building a parallel static-page generator for built-in Examples, unless there's a reason to keep some setups out of the public submission flow.

Each new page (of either kind) should contain:
...
- updated sitemap entries for every substantive example and guide — **already automated for the wiki**: the build script regenerates `sitemap.xml` with every wiki page whenever content changes. Extend the same generator to cover `/community/` entries and any new named-setup pages so this stays automatic instead of becoming a manually-maintained list.

### GitHub discoverability

**Confirmed still fully open** — the repository currently has zero topics set and no homepage URL configured, so this section is not just still valid, it's a same-day, zero-cost fix worth doing before anything else in this document (also add the homepage URL field pointing at `https://opticalsetup.com/`, which is unset too).
...
- three screenshots near the top of the README — **still missing**, confirmed by inspection.
- ~~a concise "Try it now" call to action before the feature inventory~~ — **done**: the README opens with a bolded one-line "Try it in your browser" link (plus a GitHub Pages mirror) directly under the title, before any feature description.
- `CITATION.cff` so academic users can cite the project — **still missing**.
- a tagged release with release notes — **still missing**: no GitHub releases exist yet.
- contribution and code-of-conduct files — **still missing**: no `CONTRIBUTING.md` or `CODE_OF_CONDUCT.md`.
- issue labels for `good first issue`, `optical-element`, `physics`, `documentation`, and `example-setup` — **partially done**: `good first issue` and `documentation` already exist (GitHub defaults); `optical-element`, `physics`, and `example-setup` still need to be created.
...
### Angle 1b (new): an encyclopedia that shows its work

**Claim:** Most simulation tools either don't explain the underlying physics or don't say where they simplify it. Every OpticalSetup wiki page does both side by side — the real-world formula and citation, then exactly how the app's qualitative model differs from it — for the same component you're placing on the canvas.

**Proof:** `/wiki/` — live today, 24 pages and growing, each with its own live embedded demo. This is also the strongest concrete illustration of Angle 3 below; lead with a specific page (e.g. the etalon or VIPA pages, which show genuinely non-trivial physics like multi-beam interference) rather than describing the wiki abstractly.
...
**Proof:** documented simulation scope and capability states; the wiki's "Simplified vs. reality" callout on every page, which states the gap explicitly per component rather than in one general disclaimer.
...
**Proof:** share links, read-only community pages (`/community/`, 5 live today), SVG/PNG export, and — once the queued GIF export ships — a looping animated export too, for setups where the pulsed/mechanical motion is the point.
...
- [ ] **Add GitHub repository topics and the homepage URL field** — confirmed empty today, zero-cost, do this first.
- [ ] Add a project-domain contact inbox.
- [ ] Create `/press/`.
- [ ] Record the 60–90 second demo.
- [ ] Package the 3 canonical demos that already exist (Mach–Zehnder, bright field microscope, optical spectrometer) with stable, promotable URLs; commission the 3 that are still missing (fluorescence microscopy path, telescope/camera image formation, a focused pulsed-delay-line demo).
- [ ] Add GitHub social preview.
- [ ] Add `CITATION.cff`.
- [ ] Add three screenshots to the top of the README.
...
- [ ] Once GIF export ships, record a 20–30 second looping GIF of a pulsed setup directly from the app for press/social use.
...
- [ ] Add wiki pages for the highest-search-volume components still missing one (of 34 remaining; prioritize by name-recognition — objective, polarizer, beamsplitter-family — over obscure ones).
