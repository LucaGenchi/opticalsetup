...
- **Examples menu**: pedagogical image-formation and laboratory sketches, plus a
  large NIF teaching scene assembled from standard optics with 11 live
...
It uses only the normal component library: one 1053 nm pulsed master laser, staged
...
and a qualitative drive detector. A separate 659.5 nm pulsed probe laser,
splitter/recombiner, delayed reference arm, target-return mirrors, and camera form a
traced VISAR-style diagnostic. Pulse repetition is illustrative rather than a model
of NIF shot cadence. The model reports optical return and path-delay spread, but not
coherent fringe phase or velocity. Amplifier gain and four-pass PEPC switching, the
literal 192-beam/48-port 3D cone geometry, DANTE x-ray response, neutron time of
flight, x-ray transport, implosion, fusion, yield, damage, and calibrated energetics
