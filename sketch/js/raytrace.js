  ...
  switch (k) {
    ...
    case 'lens': {
      ...
      if (Number.isFinite(data.objectiveNA) && Number.isFinite(data.objectiveMediumIndex)) {
        // The objective's effective object-space cone is a real acceptance
        // boundary shared with its clear opening and visible guide. From the rear,
  ...
  for (let i = 0; i < K - 1; i++) {
    ...
    for (const ra of bySample.get(i) || []) {
      ...
      if (rb && !rb.speckle && !isPostFocus(ra)) {
        ...
        const op = opacityOf(ra, 0.28 * Math.max(0.4 / Math.sqrt(siblings), ra.intensity));
