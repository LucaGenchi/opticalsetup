...
- `calculators/` — the Calculators section: one page per physical model,
  generated by `tools/build-calculators.mjs` from
  `tools/calculators-content.mjs`; each page's DOM-free module
  (`calculators/<slug>/`) calls the app's own functions. Rebuild after
  editing the content; the suite fails if a committed page is stale.
- `validation/` — independent Python references for the quantitative models
  (`python3 validation/run.py` regenerates `docs/validation.md`); see
  `docs/adding-physics.md`.
- `test/` — Node regression tests (import from `../sketch/js/`); they need no
  packages except the page-generator checks, which use the KaTeX dev
  dependency and skip with a reason when it is not installed.
...
```bash
npm ci               # dev dependencies (KaTeX), for the page generators
...
- `main` is protected (enabled 2026-09-24, enforced for administrators too, so
  `gh pr merge --admin` does not bypass it): changes arrive only through a
  pull request; the `CI / test` status check (`.github/workflows/ci.yml`:
  syntax, whitespace, then `npm ci` and the full suite, including the Python
  references and the generated calculator pages) must pass; the branch must be
  up to date with `main`. No approving review is required: Andrea's
  `/andrea-review` comments inform the merge but do not gate it, and Luca
  decides when to merge. A red suite therefore cannot reach the live site.
  Only Luca can change these settings (**Settings → Branches → main**); do not
  ask for them to be relaxed to get a change merged.
